...
const PODIUM_ORDER = [1, 0, 2]; // silver, gold, bronze — gold al centro
  ...
  return (
    ...
    >
      ...
      <div className="text-center mt-2">
        <p className="font-bold text-white text-sm truncate max-w-[120px]">{entry.squadra}</p>
        <p className={cn('text-xs mt-0.5', medal.bg === 'bg-yellow-400' ? 'text-yellow-400' : 'text-gray-400')}>
          {medal.label}
        </p>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {top3.length > 0 && (
              <div className="mb-10">
                <h2 className="font-bebas text-3xl text-center mb-8 tracking-wide">
                  🏆 Podio Top 3
                </h2>
                <div className="flex items-end justify-center gap-4">
                  {PODIUM_ORDER.map((rank) => {
                    ...
                    return (
                      <PodiumCard key={entry.id} entry={entry} rank={rank} isCurrentUser={isCurrentUser} />
                    );
                  })}
                </div>
              </div>
              ...
              {top3.map((entry, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      'grid grid-cols-12 items-center px-6 py-4 border-b border-white/5 transition-colors',
                      isCurrentUser ? 'bg-primary/10' : 'hover:bg-white/5'
                    )}
                  >
                    <div className="col-span-1 text-lg">{medal.emoji}</div>
                    <div className="col-span-7">
                      <span className={cn('font-bold', isCurrentUser ? 'text-primary' : 'text-white')}>
                        {entry.squadra}
                      </span>
                      {isCurrentUser && <span className="ml-2 text-xs text-primary font-bold">Tu</span>}
                    </div>
                    <div className="col-span-2 text-center">
                      <span className={cn('px-2 py-1 rounded-lg text-xs font-bold', medal.bg, medal.text)}>
                        {entry.punti}
                      </span>
                    </div>
                    <div className="col-span-2 text-center">
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      isCurrentUser ? 'bg-primary/10' : 'hover:bg-white/5',
                      isLast ? 'border-l-4 border-l-red-500' : ''
                    )}
                  >
                    <div className="col-span-1 font-bebas text-xl text-muted-foreground">{pos}</div>
                    <div className="col-span-7">
                      <span className={cn('font-medium', isCurrentUser ? 'text-primary' : 'text-white')}>
                        {entry.squadra}
                      </span>
                      {isCurrentUser && <span className="ml-2 text-xs text-primary font-bold">Tu</span>}
                    </div>
                    <div className="col-span-2 text-center">
                      <span className="px-2 py-1 rounded-lg text-xs font-bold bg-white/10 text-white">
                        {entry.punti}
                      </span>
                    </div>
                    <div className="col-span-2 text-center">
                      <span className="px-2 py-1 rounded-lg text-xs font-bold bg-white/5 text-muted-foreground">
                        {entry.posizione}
                      </span>
                    </div>
                  </motion.div>
