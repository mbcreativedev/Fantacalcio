...
const getAvatarUrl = (user) => {
  ...
  return pb.files.getURL(user, user.avatar, { thumb: '300x300' });
};
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        try {
          const res = await pb.collection('iscrizioni').getList(1, 50, {
        ...
        try {
          const res = await pb.collection('classifiche').getList(1, 50, {
            filter: `user_id="${id}"`,
            sort: '-punti',
            $autoCancel: false,
          });
          setClassifiche(res.items);
        } catch (e) { setClassifiche([]); }
  ...
  return (
    <div className="min-h-screen bg-background py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Button variant="ghost" className="mb-8 text-muted-foreground hover:text-primary -ml-4" onClick={() => navigate(-1)}>
          <ArrowLeft className="mr-2 h-4 w-4" /> Torna indietro
        </Button>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-10">
            <div className="h-28 w-28 rounded-full bg-primary/20 flex items-center justify-center border-2 border-primary overflow-hidden flex-shrink-0">
              {avatarUrl ? (
            ...
            <div className="text-center sm:text-left">
              <h1 className="font-bebas text-4xl tracking-wide">{user.name || 'Giocatore'}</h1>
              <p className="text-primary font-medium text-lg mt-1">{user.squadra || 'Nessuna squadra'}</p>
              {isOwnProfile && (
                <div className="mt-3 flex items-center gap-2 flex-wrap justify-center sm:justify-start">
                  <span className="px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold border border-primary/20">Il tuo profilo</span>
                  <Button size="sm" variant="outline" className="border-white/10 hover:text-primary text-xs h-7" onClick={() => navigate('/profilo')}>
                    Modifica profilo
                  </Button>
                </div>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <Card className="bg-card border-white/10">
              ...
              <CardContent>
                {enrollments.length === 0 ? (
                  <p className="text-muted-foreground text-sm text-center py-6">Nessun torneo attivo.</p>
                ) : (
                  <div className="space-y-3">
                    {enrollments.map((e) => {
                      const torneo = e.expand?.torneo_id;
                      return (
                        <div key={e.id} className="flex justify-between items-center p-3 bg-background/50 rounded-lg border border-white/5 hover:border-primary/20 transition-colors cursor-pointer" onClick={() => navigate(`/tournament/${torneo?.id}`)}>
                          <span className="font-medium text-foreground">{torneo?.nome || 'Torneo'}</span>
                          <span className="text-xs px-2 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">{e.stato}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            <Card className="bg-card border-white/10">
              <CardHeader>
                <CardTitle className="font-bebas text-2xl">🏆 Risultati</CardTitle>
              </CardHeader>
              <CardContent>
                {classifiche.length === 0 ? (
                  <p className="text-muted-foreground text-sm text-center py-6">Nessun risultato disponibile.</p>
                ) : (
                  <div className="space-y-3">
                    {classifiche.map((c) => (
                      <div key={c.id} className="flex justify-between items-center p-3 bg-background/50 rounded-lg border border-white/5">
                        <div>
